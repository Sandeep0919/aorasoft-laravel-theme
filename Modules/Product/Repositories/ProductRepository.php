...
class ProductRepository
{
    ...
    public function create(array $data)
    {
        ...
        if(isModuleActive('AuctionProducts')){
            $rangeArr = $data['date_range'] ? explode('-', $data['date_range']) : "" . date('m/d/Y h.i.A') . " - " . date('m/d/Y h.i.A') . "";
            $date_from = date('d-m-Y h.i.A', strtotime($rangeArr[0]));
            $date_to = date('d-m-Y h.i.A', strtotime($rangeArr[1]));
            $data['auction_products'] = $data['auction_products'];
            $data['auction_start_date'] = $date_from;
            $data['auction_end_date'] = $date_to;
        }
    ...
    {
        return SellerProductSKU::with('products','mainProducts')->where('status', 1)->where(['user_id' => $seller_id, 'product_id'=> $id])->first();
    }
    public function findSkus($id)
    {
        return SellerProductSKU::with('products','mainProducts')->where('status', 1)->find($id);
    }
}
